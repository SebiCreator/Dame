...
import de.htwg.se.dame.model.gameComponent.LinesInterface

object Lines extends LinesInterface {
...
// Lines & Blocks with symbols
  ...
  def bLineFull(cellsize: Int, nFields: Int, symbol: String) =
    "|" + (innerBCell(
      cellsize,
      symbol
    ) + innerWCell(cellsize, symbol)) * halfFloor(nFields) + "|" + eol
  def wLineFull(cellsize: Int, nFields: Int, symbol: String) =
    "|" + (innerWCell(
      cellsize,
      symbol
...
// alle Argumente möglich ( cellsize:  wieviel pix um symbol, nFields Anzahl der Doppelfelder)
  def fullBoardWrapped2(cellsize: Int, nFields: Int, symbol: String) =
    limitWrap(
      makeodd(cellsize),
      nFields * 2,
      fullBoard(makeodd(cellsize), nFields * 2, symbol)
